...
export default function CurrentConditionsCard({
  ...
  const detailRows: DetailRow[] = [
    ...
    },
    {
      ...
      label: "Wind",
      value: `${convertWind(current.wind_speed, windUnit)} ${windUnitLabel(windUnit)} ${degreesToCompass(current.wind_direction)}`,
    },
    {
        ...
        >
          <Box>
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
              <Box sx={{ color: current.is_day ? "warning.main" : "info.main", display: "flex" }}>
                {current.is_day
              ...
              <Box>
                <Typography variant="h2" sx={{ fontWeight: 700, lineHeight: 1 }}>
                  {convertTemp(current.temperature, temperatureUnit)}{tempUnitLabel(temperatureUnit)}
                </Typography>
                <Typography variant="body1" sx={{ color: "text.secondary", mt: 0.5 }}>
                  {label} · Feels like {convertTemp(current.apparent_temperature, temperatureUnit)}{tempUnitLabel(temperatureUnit)}
                </Typography>
              </Box>
            </Box>
          </Box>
